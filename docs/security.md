...
| **SR-019** | Critical | Open | No production environment — zero ability to verify security controls in prod | GCP project `smartcommission-prod` now exists (gcloud returns empty log array rather than permission denied). Cloud Run not yet deployed. Firebase Auth not yet activated. All security controls remain code-only. Cross-tenant isolation tests not yet implemented. Must verify every control before first user data is stored. Last checked: 2026-06-22. |
