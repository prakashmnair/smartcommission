...
| **SR-022** | High | ✅ Fixed 2026-06-24 | Cloud Build SA missing Secret Manager access — secrets exposed to build environment without proper IAM controls | Build `46e3013e` (2026-06-24) **SUCCEEDED** — IAM binding was applied between builds. CI/CD pipeline unblocked. Cloud Run service deployed. See I-012, I-013 (both closed). |
| **SR-023** | Low | Open | 31 npm audit moderate vulnerabilities via `uuid` in `firebase-admin` dependency chain | `firebase-admin@>=11.0.0` depends on a vulnerable `uuid` version. No HIGH/CRITICAL CVEs. Cannot force-upgrade firebase-admin without breaking to v10 (breaking change). Monitor for firebase-admin patch that resolves. See S-022. |
| **SR-024** | Medium | Open | `/api/health` endpoint was returning 404 — uptime checks ineffective | `/api/health` did not exist. GCP Monitoring uptime check was hitting a 404. Fixed 2026-06-26: endpoint created, returns 200/503 based on DB probe. GCP uptime check `smartcommission-api-health-bkZvLcNxj_k` registered. Alert policy still needs to be linked (see I-015). |
