...
| **SR-013** | Critical | Open | SecurityLog model not yet implemented | `SecurityLog` Prisma model and `lib/security-log.ts` required by CLAUDE.md. Auth events (login, logout, password reset) and permission changes (SUPERADMIN_GRANTED) are not being logged. Must be implemented before any auth routes are built. |
| **SR-014** | Critical | Open | Superuser pattern not yet implemented | `isSuperAdmin()` helper with hardcoded `prakashmnair@gmail.com`, self-revoke protection, and superadmin console route guard are not yet built. Required by CLAUDE.md before launch. |
| **SR-015** | High | Open | No application-layer Prisma organisationId middleware yet | The Prisma middleware that validates `organisationId` is present on all tenanted table queries (primary defence against cross-tenant leaks) is planned but not implemented. |
