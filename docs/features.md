...
| **I-012** | Critical | Open | Cloud Build SA missing Secret Manager access — CI/CD pipeline blocked | Build `429b3a22` (2026-06-21) **FAILED**: `Permission 'secretmanager.versions.access' denied on resource projects/smartcommission-prod/secrets/smartcommission-firebase-api-key`. Cloud Build SA `1028287218164@cloudbuild.gserviceaccount.com` does not have `roles/secretmanager.secretAccessor` on the Firebase secrets used in build Step 0. Fix: `gcloud secrets add-iam-policy-binding <secret-name> --member="serviceAccount:1028287218164@cloudbuild.gserviceaccount.com" --role="roles/secretmanager.secretAccessor" --project=smartcommission-prod` for all secrets in `cloudbuild.yaml`. Blocks all deploys. |
